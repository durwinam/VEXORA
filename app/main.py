...
app.mount('/static',StaticFiles(directory='/opt/vexora/app/static' if Path('/opt/vexora/app/static').exists() else Path(__file__).parent/'static'),name='static')
...
def buy(plan_id:int,request:Request,username:str=Form(...)):
    ...
    return RedirectResponse('/shop/receipt/'+str(c),303)
...
@app.get('/admin/panels',response_class=HTMLResponse)
...
def add_panel(request:Request,name:str=Form(...),kind:str=Form(...),base_url:str=Form(...),username:str=Form(''),secret:str=Form('')):
    ...
    pid=exec('INSERT INTO panels(tenant_id,name,kind,base_url,username,secret_enc) VALUES(?,?,?,?,?,?)',(a['tenant_id'],name,kind,base_url,username,encrypt(secret) if secret else None)); audit(a['id'],'panel_created',request.client.host if request.client else '',{'panel_id':pid}); return RedirectResponse('/admin/panels',303)
...
def add_plan(request:Request,name:str=Form(...),panel_id:int=Form(...),price:int=Form(...),volume_gb:float=Form(...),days:int=Form(...)):
    ...
    exec('INSERT INTO plans(tenant_id,panel_id,name,price,volume_gb,days) VALUES(?,?,?,?,?,?)',(a['tenant_id'],panel_id,name,price,volume_gb,days)); return RedirectResponse('/admin/plans',303)
...
def review(rid:int,request:Request,decision:str=Form(...)):
    ...
    return RedirectResponse('/admin/receipts',303)
...
def add_admin(request:Request,username:str=Form(...),password:str=Form(...),role:str=Form(...)):
    ...
    exec('INSERT INTO admins(username,password_hash,role,tenant_id) VALUES(?,?,?,?)',(username,hash_password(password),role,a['tenant_id'])); return RedirectResponse('/admin/admins',303)
