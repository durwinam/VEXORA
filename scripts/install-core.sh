...
source_root() {
    ...
    base="$(source_dir)"
    if [[ -d "${base}/app" && -f "${base}/install.sh" ]]; then
        printf '%s\n' "${base}"
        return 0
    fi
    candidate="$(find "${base}" -mindepth 2 -maxdepth 3 -type f -name install.sh -print -quit 2>/dev/null || true)"
    if [[ -n "${candidate}" && -d "$(dirname "${candidate}")/app" ]]; then
        printf '%s\n' "$(dirname "${candidate}")"
        return 0
    fi
    return 1
...
prepare() {
    local src
    src="$(source_root)" || fail "Incomplete VEXORA source: app/ is missing. Extract the complete VEXORA 1.0.0 project before running install.sh."
    ...
    for required_script in backup.sh restore.sh update.sh uninstall.sh; do
        [[ -f "${src}/scripts/${required_script}" ]] || fail "Incomplete VEXORA source: scripts/${required_script} is missing."
    done
...
install_source() {
    local src
    src="$(source_root)" || fail "Unable to locate VEXORA source root."
...
certificate() {
    ...
    if ss -lnt | grep -qE ":80[[:space:]]"; then
        fail "Port 80 is still in use. Stop the service using port 80 so ACME HTTP-01 can run."
    fi
...
PY
    ...
    # Keep the stable VEXORA certificate paths synchronized after renewal.
    install -d -m 0755 /etc/letsencrypt/renewal-hooks/deploy
    cat > /etc/letsencrypt/renewal-hooks/deploy/vexora-cert-sync.sh <<EOF
#!/usr/bin/env bash
set -Eeuo pipefail
CERT_LIVE_DIR="/etc/letsencrypt/live/${CERT_ID}"
CERT_DIR="${CERT_DIR}"
...
EOF
    chmod 0755 /etc/letsencrypt/renewal-hooks/deploy/vexora-cert-sync.sh
}
...
nginx_config() {
    ...
    sed \
        ...
        -e "s/listen \[::\]:8443 ssl/listen [::]:${ALT_HTTPS_PORT} ssl/" \
        -e "s/server_name _;/server_name ${CERT_ID};/g" \
